...
class ListUpdateBody(BaseModel):
    update: ListUpdate = Field(
        description="Whether this operation should add or remove an element")


class Success(BaseModel):
    success: bool = Field(
        description="Whether the action succeded", default=True)


class UserData(BaseModel):
    likes: List[UUID] = Field(description="The videos this user likes")
    watch_later: List[UUID] = Field(
        description="The videos this user has added to their \"watch later\" list")
